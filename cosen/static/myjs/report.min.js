...
function ReportViewModel() {
    ...
    self.showRequest = function (formData, jqForm, options) {
        ...
        if (self.cosen["hdpagenum"].val() === "1") {
            self.cosen.report.html("<tr><td colspan='14'>正在查询...</td></tr>")
        }
    ...
    self.showResponse = function (responseText, statusText) {
        ...
        if (responseText.length === 0) {
            ...
            if (pagenum === 1) {
                self.cosen.report.html("<tr><td colspan='14'>没有查到数据</td></tr>");
            }
            ...
            $.each(responseText, function (index, value) {
                self.htmlStr.push("<tr><td>" + (pindex + index + 1) +
                    ...
                    "</td><td><input ondblclick='editDate(this);' id='" + value.nsty_no + "' type='number' max='12' min='01' class='form-control input-sm' readonly value='" + (value.editionhandle === null ? "" : value.editionhandle) + "'/></td><td><img src='/static/images/single/" + value.nsty_no +
...
function editDate(obj) {
    var input = $(obj), style_no = input.attr("id"), btn = "#btn_" + style_no;
    input.removeAttr('readonly');
    $(btn).removeAttr("disabled");
}
//保存日期
function saveDate(obj) {
    var btn = $(obj), style_no = btn.attr("id").split('_')[1], input = $("#" + style_no);
    var month = parseInt(input.val());

    $.post('/report/updatedate', { styleNo: style_no, date: month < 10 ? "0" + month.toString() : month.toString() }, function (data, textStatus, jqXHR) {
        if (data === "success") {
